# Certification Requirements — Flight Training Scheduler

> Original certification brief as provided by Akka. Reproduced here for reference.
...
If a timeslot has a given booking then that booking can be canceled. The call to the HTTP endpoint's "create booking" route requires the client to pass the booking ID so it will be able to use it for future calls such as `cancel`.
...
```bash
# Mark availability for 3 participants
...
curl -v -H "Content-Type: application/json" \
  -d '{"bookingId": "booking4", "aircraftId": "superplane", "instructorId": "superteacher", "studentId": "alice"}' \
  localhost:9000/flight/bookings/2025-12-10-10

# Cancel the booking
curl -v -X DELETE localhost:9000/flight/bookings/2025-12-10-10/booking4
```
